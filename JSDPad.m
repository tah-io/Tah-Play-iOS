...
- (void)commonInit
{
	_dPadImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"control.png"]];
...
- (JSDPadDirection)directionForPoint:(CGPoint)point
{
	...
	int column = x / ([self bounds].size.width / 3);
	int row = y / ([self bounds].size.height / 3);
...
- (UIImage *)imageForDirection:(JSDPadDirection)direction
{
	...
	switch (direction) {
		case JSDPadDirectionNone:
			image = [UIImage imageNamed:@"control.png"];
			break;
